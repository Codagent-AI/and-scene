...
try {
  ...
  const progress = page.locator('[data-presentation-progress-item]')
  if (await progress.count()) await progress.first().click()
  await page.waitForFunction((expected) => Number(document.querySelector('[data-step-index]')?.getAttribute('data-step-index')) === expected, 0)
  for (let index = 0; index < count; index += 1) {
    if (await progress.count()) await progress.nth(index).click()
    else if (index > 0) await page.keyboard.press('ArrowRight')
