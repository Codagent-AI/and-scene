...
export function validateReferenceOutline(actual) {
  ...
  for (let index = 0; index < Math.min(actual.length, referenceOutline.length); index++) {
    ...
    if (!step || step.era !== era) errors.push(`step ${index + 1} era must be "${era}"`)
    if (!step || step.title !== title) errors.push(`step ${index + 1} title must be "${title}"`)
    if (!step || step.caption !== caption) errors.push(`step ${index + 1} caption must match the canonical outline`)
  }
  return errors
}
