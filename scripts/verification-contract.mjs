...
export function validateReferenceContract(registrySource, stepSource) {
  ...
  let cursor = -1
  for (const [, title, caption] of canonicalSteps) {
    const titleAt = stepSource.indexOf(title, cursor + 1)
    const captionAt = stepSource.indexOf(caption, titleAt + 1)
    const stepNumber = canonicalSteps.findIndex((step) => step[1] === title) + 1
    if (titleAt < 0 || captionAt < 0) throw new Error(`Sample contract check failed at step ${stepNumber}: expected canonical title and caption in order`)
    cursor = captionAt
  }
}
