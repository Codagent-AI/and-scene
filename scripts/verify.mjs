...
async function renderEveryStep() {
  ...
  try {
    ...
    if (count !== canonicalSteps.length) throw new Error(`render phase failed: expected ${canonicalSteps.length} steps, received ${count}`)
...
try {
  ...
  await renderEveryStep()
  console.log(`verify: PASS — ${canonicalSteps.length} reference steps rendered cleanly on 127.0.0.1`)
} catch (error) {
