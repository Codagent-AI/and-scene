...
Use the generic kit primitives and hooks. The reusable kit owns behavior and geometry, not visual style. Fit scaling reserves the header, footer, and side space in the kit's `STAGE_LAYOUT` (plus a taller browse footer at narrow widths in `useFitScale`), so keep presentation or host chrome and `.presentation-stage` insets within those reservations. Put the designed look in the presentation's own plain CSS (or the host's existing styling system): own colors, typography, spacing, borders, shadows, and control treatments there. Make captions readable, active progress or table-of-contents state obvious, and attribution legible. Mark overlap as intentional only when it remains readable.
