  ...
  try {
    ...
    for (let index = 0; index < Number(count); index += 1) {
      ...
      const diagnostics = await page.evaluate(() => {
        ...
        const overlaps = (a, b) => { const x = a.getBoundingClientRect(); const y = b.getBoundingClientRect(); return x.left < y.right && x.right > y.left && x.top < y.bottom && x.bottom > y.top }
        const allowed = (element) => Boolean(element.closest('[data-presentation-allow-overlap="true"]'))
        const collisions = []
        for (let a = 0; a < textElements.length; a += 1) for (let b = a + 1; b < textElements.length; b += 1) if (!allowed(textElements[a]) && !allowed(textElements[b]) && overlaps(textElements[a], textElements[b])) collisions.push(`${textElements[a].textContent?.trim().slice(0, 30)} / ${textElements[b].textContent?.trim().slice(0, 30)}`)
