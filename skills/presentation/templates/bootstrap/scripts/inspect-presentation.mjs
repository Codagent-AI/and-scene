...
async function inspectStep(page, index) {
  const warnings = await page.evaluate(() => {
    ...
    if (!attribution) result.push('missing [data-presentation-attribution] attribution')
      ...
      if (Number.parseFloat(style.fontSize) < 10) result.push('attribution is undersized; style [data-presentation-attribution] locally')
...
async function main() {
  ...
  const preview = spawn('npm', ['run', 'preview', '--', '--host', host, '--port', String(port), '--strictPort'], { stdio: 'inherit', shell: process.platform === 'win32', detached: process.platform !== 'win32' })
  let browser
  try {
