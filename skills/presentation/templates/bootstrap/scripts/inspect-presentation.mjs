...
const child = spawn(process.execPath, ['node_modules/vite/bin/vite.js', 'preview', '--host', '127.0.0.1', '--port', '4179', '--strictPort'], { stdio: 'ignore' })
let browser
try {
  let ready = false
  for (let i = 0; i < 60; i++) { try { ready = (await fetch('http://127.0.0.1:4179/')).ok; if (ready) break } catch {} await delay(250) }
  ...
  await page.goto(`http://127.0.0.1:4179/${encodeURIComponent(slug)}`, { waitUntil: 'networkidle' })
  ...
  for (let index = 0; index < count; index++) {
    ...
    const diagnostics = await page.evaluate(() => {
      ...
      const active = document.querySelector('[data-presentation-progress-item][aria-current="step"]')
      const inactive = document.querySelector('[data-presentation-progress-item]:not([aria-current="step"])')
      const activeStyle = active && getComputedStyle(active)
      const inactiveStyle = inactive && getComputedStyle(inactive)
      const activeDistinct = Boolean(active && (!inactive || active.getAttribute('data-presentation-active') === 'true') && (
    ...
    if (!diagnostics.activePresent || !diagnostics.activeDistinct) console.warn(`WARN step ${index + 1}: active progress state is missing or visually indistinct`)
