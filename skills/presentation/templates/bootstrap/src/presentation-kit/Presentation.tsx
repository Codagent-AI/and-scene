...
export default function Presentation<T>({ steps, title, initialMode = 'browse', designWidth = DESIGN_W, designHeight = DESIGN_H, className = '', style, branding, attribution }: PresentationProps<T>) {
  ...
  return <main className={`presentation presentation--${mode} ${className}`} style={{ position: 'fixed', inset: 0, ...style }} data-presentation="" data-presentation-mode={mode} data-step-count={steps.length} data-step-index={index} data-step-title={step.title} data-step-caption={step.caption} onTouchStart={(event) => { touch.current = { x: event.touches[0].clientX, y: event.touches[0].clientY } }} onTouchEnd={(event) => {
    ...
    <footer className="presentation-footer" data-presentation-footer="">
      {mode === 'browse' ? <><p data-presentation-caption="">{step.caption}</p><nav aria-label="Presentation progress" data-presentation-progress="">{steps.map((item, i) => <button type="button" key={item.id} data-presentation-progress-item="" data-presentation-active={i === index ? 'true' : 'false'} aria-current={i === index ? 'step' : undefined} aria-label={`Go to step ${i + 1}: ${item.title}`} onClick={() => goTo(i)}>{i + 1}</button>)}</nav><button type="button" data-presentation-prev="" onClick={prev} disabled={index === 0}>Previous</button><button type="button" data-presentation-next="" onClick={next} disabled={index === steps.length - 1}>Next</button><button type="button" data-presentation-toc-toggle="" onClick={() => setTocOpen((open) => !open)} aria-expanded={tocOpen}>Contents</button>{(tocOpen || wide) && <nav aria-label="Table of contents" data-presentation-toc="">{eras.map(({ era, index: eraIndex }) => <button type="button" key={era} data-presentation-toc-item="" data-presentation-active={steps[index].era === era ? 'true' : 'false'} aria-current={steps[index].era === era ? 'location' : undefined} onClick={() => goTo(eraIndex)}>{era}</button>)}</nav>}</> : <div data-presentation-present-title="">{step.title}</div>}
