...
export function Presentation<T>({ steps, title, initialMode = 'browse', branding, className }: PresentationProps<T>) {
  ...
  return <main className={`presentation ${className ?? ''}`} data-presentation-mode={nav.mode} data-step-count={steps.length} data-step-index={nav.index} onTouchStart={touchStart} onTouchEnd={touchEnd}>
    <header className="presentation-header" data-presentation-header="">
      ...
      <div className="presentation-heading"><span data-presentation-marker="">{step.era} · {String(nav.index + 1).padStart(2, '0')}</span>{nav.mode === 'browse' && <strong>{title}</strong>}</div>
      <button type="button" data-presentation-mode-toggle="" aria-label={`Switch to ${nav.mode === 'browse' ? 'present' : 'browse'} mode`} onClick={nav.toggleMode}>{nav.mode === 'browse' ? 'Present' : 'Browse'}</button>
    </header>
    <Stage step={step} mode={nav.mode} />
    <footer className="presentation-footer" data-presentation-footer="">
      <div className="presentation-narration">{nav.mode === 'present' ? <strong>{step.title}</strong> : <><strong>{step.title}</strong><p>{step.caption}</p></>}</div>
      {nav.mode === 'browse' && <>
