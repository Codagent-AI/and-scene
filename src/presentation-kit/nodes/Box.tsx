import { motion } from 'motion/react'
import type { CSSProperties, ReactNode } from 'react'
import { ENTER_DELAY, ENTER_T, LAYOUT_T } from '../constants'
export function Box({ id, children, className = '', style, ...props }: { id: string; children: ReactNode; className?: string; style?: CSSProperties; [key: `data-${string}`]: string | undefined }) {
  return <motion.div layout layoutId={id} className={`scene-box ${className}`.trim()} data-presentation-node="box" data-entity-id={id} style={style} initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { opacity: { duration: ENTER_T, delay: ENTER_DELAY } } }} exit={{ opacity: 0, transition: { duration: ENTER_T } }} transition={{ layout: LAYOUT_T }} {...props}>{children}</motion.div>
}
