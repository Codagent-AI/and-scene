...
function StepCard({ index }: { index: number }) {
  return (
    <Appear className={`sample-card-enter sample-card-enter-${index}`}>
      <Box layoutId={entities.cards[index]} className={`sample-step-card sample-step-card-${index}`}>
        <span className="sample-card-number">0{index + 1}</span>
        <span>{CARD_NAMES[index]}</span>
      </Box>
    </Appear>
  )
}

function visibleCardCount(beat: number) {
  if (beat === 3) return 1
  if (beat === 4) return 4
  return beat >= 3 ? 5 : 0
}
...
export function HowToScene({ payload }: SceneProps<SamplePayload>) {
  ...
  const cardCount = visibleCardCount(beat)
  return <SceneLayer className="how-to-scene">
    ...
    {Array.from({ length: cardCount }, (_, index) => <StepCard key={entities.cards[index]} index={index} />)}
    {Array.from({ length: Math.max(0, cardCount - 1) }, (_, index) => (
      <Appear key={entities.cardLinks[index]}>
        <Arrow layoutId={entities.cardLinks[index]} className={`sample-card-link sample-card-link-${index}`} />
      </Appear>
    ))}
    ...
    {beat >= 8 ? (
      <div data-presentation-allow-overlap>
        <Appear><Arrow layoutId={entities.modify} className="sample-modify-arc" /></Appear>
        <Appear>
          <SymbolChip layoutId={entities.edited} className="sample-edited" Icon={Pencil}>edited</SymbolChip>
        </Appear>
      </div>
    ) : null}
    {beat >= 9 ? (
      <div data-presentation-allow-overlap>
        <Appear>
          <Frame layoutId={entities.reveal} className="sample-reveal"><span>you’re looking at one</span></Frame>
        </Appear>
      </div>
    ) : null}
  </SceneLayer>
