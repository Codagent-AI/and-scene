...
export function Scene({ index }: { index: number }) {
  ...
  return <div className="howto-scene" data-howto-scene>
    <Label id={ENTITY.you} className="entity you" style={{ left: 30, top: 26 }}>YOU</Label>
    <Box id={ENTITY.prompt} className="entity prompt" style={{ left: 144, top: 22 }}>“I have a topic…”</Box>
    {index >= 1 && <>
    ...
    {index >= 2 && <div className="card-tray">{cards.slice(0, Math.max(1, count)).map(([number, title, detail], i) =>
      <Box key={number} id={`howto-card-${number}`} className={`step-card entity ${index === 7 && i === 1 ? 'edited' : ''}`} style={{ left: 20 + i * 138, top: 116 }}>
        ...
        {index >= 2 && <em>title · caption · visual</em>}
      </Box>)}
    ...
    {index === 4 && <>
      <Box id="howto-ghost" className="step-card ghost entity" style={{ left: 20 + count * 138, top: 116 }}>… your next beat</Box>
    ...
    {index >= 5 && <>
      ...
      <Box id={ENTITY.kit} className="kit-socket entity" style={{ left: 340, top: 250 }}>◈ &nbsp; SHARED SCENE KIT</Box>
    </>}
    {index >= 6 && <>
      ...
      <Box id={ENTITY.verify} className="verify-node entity" style={{ left: 766, top: 128 }}>✓ &nbsp; BUILD + RENDER</Box>
    </>}
    {index >= 7 && <Emphasis id={ENTITY.modify} className="modify-arc entity" style={{ left: 365, top: 294 }}>↶ &nbsp; CHANGE A BEAT · EDIT IN PLACE</Emphasis>}
    {index >= 8 && <Frame className="reveal-frame entity" data-presentation-node="reveal-frame" data-presentation-allow-overlap="true"><span>YOU ARE INSIDE THE EXAMPLE</span></Frame>}
    <div className="scene-baseline" aria-hidden="true" />
  </div>
}
