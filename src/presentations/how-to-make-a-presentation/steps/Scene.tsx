...
export function ReferenceScene({ payload }: SceneProps<ReferencePayload>) {
  ...
  return <SceneLayer className="reference-scene">
    <div className="conversation">
      ...
      {through >= 1 && <Box id={entity('skill')} className="ref-person ref-skill" style={{ left: 672, top: 22 }}>skill</Box>}
      {through >= 1 && <Arrow id={entity('conversation-link')} className="ref-link" style={{ left: 382, top: 49 }} label="Two-way conversation" />}
      {through >= 1 && <SymbolChip id={entity('question')} className="ref-question" style={{ left: 394, top: 4 }}>one question at a time</SymbolChip>}
    </div>

    <div className="step-tray" aria-label="Accumulating presentation steps">
      {cards.slice(0, cardCount).map(([title, detail], index) => (
        <Box key={index} id={entity(`step-${index + 1}`)} className={`ref-card ${index === through ? 'ref-card-current' : ''}`} style={{ left: 34 + (index % 3) * 276, top: 108 + Math.floor(index / 3) * 67 }}>
          <span className="ref-card-number">0{index + 1}</span><strong>{title}</strong><small>{detail}</small>
        </Box>
      ))}
      {through >= 3 && <Arrow id={entity('morph-link')} className="ref-morph-link" style={{ left: 242, top: 139 }} label="What morphs between adjacent steps" />}
      {through >= 3 && <Arrow id={entity('extend-link')} className="ref-morph-link" style={{ left: 518, top: 139 }} label="The story extends" />}
      {through === 4 && <>
      ...
      {through >= 5 && <div className="kit-socket" data-presentation-allow-overlap>
      ...
      {through >= 6 && <div className="verify-node" style={{ left: 545, top: 338 }}>
      ...
      {through >= 8 && <Frame id={entity('reveal')} className="ref-reveal" label="a presentation made with this skill"><Label id={entity('reveal-note')}>one evolving scene · nine named steps</Label></Frame>}
    </div>
  </SceneLayer>
}
