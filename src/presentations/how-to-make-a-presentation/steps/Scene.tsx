...
export function Scene({ index }: { index: number }) {
  ...
  return <SceneLayer className="howto-scene" data-howto-scene="true">
    ...
    {index >= 2 && <SceneLayer key="tray" className="card-tray">{cards.slice(0, Math.max(1, count)).map(([number, title, detail], i) =>
    ...
    {index >= 7 && <Emphasis key="modify" id={ENTITY.modify} className="modify-arc entity" style={{ left: 365, top: 294 }}>↶ &nbsp; CHANGE A BEAT · EDIT IN PLACE</Emphasis>}
