...
export function StoryScene({ index }: SceneProps<number>) {
  ...
  return <div className="story" data-scene-progress={index}>
    <div className="conversation">
      ...
      <Presence>
        {index >= 1 && [
          <Arrow key="conversation-arrow" id={entity('conversation-arrow')} className="conversation-arrow" />,
          <SymbolChip key="question" id={entity('question')} className="question">one question at a time</SymbolChip>,
        ]}
      </Presence>
    </div>
    <Presence>
      {index >= 2 && [
        <motion.div key="route-label" className="route-label" exit={EXIT}><span>THE STORY ROUTE</span><span>answers become a scene <i>↘</i></span></motion.div>,
        <motion.div key="tray" className="tray" exit={EXIT}>
      ...
      {index >= 4 && [
        <Emphasis key="depth-link" id={entity('depth-link')} className="depth-link" />,
        <SymbolChip key="depth-control" id={entity('depth-control')} className="depth-control"><b>YOU CHOOSE</b><span>partial <i>↔</i> full</span></SymbolChip>,
      ]}
      {index >= 7 && [
        <motion.svg key="modify-arc" className="modify-arc" viewBox="0 0 880 380" aria-hidden="true" exit={EXIT}><path d="M 115 106 C 130 225, 215 255, 282 278" /></motion.svg>,
        <Label key="modify-label" id={entity('modify-label')} className="modify-label">EDIT IN PLACE ↶</Label>,
        <Emphasis key="edited-card" id={entity('edited-card')} className="edited-card" />,
      ]}
      {index >= 8 && [
        <Frame key="reveal-frame" id={entity('reveal-frame')} className="reveal-frame" />,
        <Label key="reveal-label" id={entity('reveal-label')} className="reveal-label">A PRESENTATION MADE WITH THIS SKILL</Label>,
      ]}
    </Presence>
  </div>
}
