...
import { REFERENCE_STEPS } from './how-to-make-a-presentation/steps'

describe('reference presentation contract', () => {
  it('registers the canonical nine step titles and captions in order', () => {
    expect(presentations.map(({ slug }) => slug)).toContain('how-to-make-a-presentation')
    expect(REFERENCE_STEPS.map(({ title, caption }) => [title, caption])).toEqual([
